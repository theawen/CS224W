...
def get_dist_data(args, graph):
    ...
    f1_name = 'datasets/cache/' + dataset + str(args.approximate) + '_dists.dat'
        
    if args.use_cache and os.path.isfile(f1_name):
        with open(f1_name, 'rb') as f1:
            dists = pickle.load(f1)
    else:
        edges = torch.stack(graph.edges())
        dists = precompute_dist_data(edges.numpy(), graph.number_of_nodes(), approximate=args.approximate)
        with open(f1_name, 'wb') as f1:
            pickle.dump(dists, f1)
            
    graph.dists = torch.from_numpy(dists).float()
    return graph
...
def load_data(args, dataset):
    ...
    ## get data with dist
    print("get started")
    graph = get_dist_data(args, graph)
...
def main():
    ...
    argparser.add_argument("--use-cache", type=bool, default=True, help="use cached data")
    argparser.add_argument("--approximate", type=int, default=-1, help="k-hop shortest path distance. -1 means exact shortest path") # -1, 2
